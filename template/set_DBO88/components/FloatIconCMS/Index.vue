<template>
  <div v-if="floatingIconList?.length > 0" class="float-wrapper">
    ...
    <div @click="toggleShow" class="icon-container">
      <q-img
        v-if="!isLoading"
        :src="isShow ? floatCmsCloseIconDarkBlue : floatIconPath"
...
const { floatCmsCloseIconDarkBlue, floatCmsOpenIconDarkBlue } = useSiteImg()
...
const floatIconPath = computed(() => {
  ...
  return matched && matched.storage_key ? matched.storage_key : floatCmsOpenIconDarkBlue
})
onMounted(() => {
