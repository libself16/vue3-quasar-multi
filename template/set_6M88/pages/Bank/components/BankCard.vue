<template>
  <q-card class="card" flat :class="{ active: props.activeId === props.card.id }">
    <div class="card-header text-black">
      {{ props.card.name }}
    </div>
    <q-card-section class="flex flex-col py-3 px-7 gap-2">
      ...
      <div class="title-row" v-show="props.card?.payment_gateway_id">
...
@keyframes glowing-border {
  0% {
    box-shadow: 0 0 0 0 rgba(#005f79, 0.7);
  }
  50% {
    box-shadow: 0 0 10px 3px rgba(#005f79, 0.7);
  }
  100% {
    box-shadow: 0 0 0 0 rgba(#005f79, 0.7);
  }
...
.card {
  ...
  &:focus {
    ...
    box-shadow: 1px 1px 10px 1px #005f79;
  }
  ...
  .title-row {
    @apply flex items-center gap-6 text-base;
