<template>
  <q-dialog v-model="dialog" persistent>
    <q-card
      class="age-warning-wrapper overflow-x-hidden overflow-y-auto"
      :class="cx('age-warning-wrapper', 'overflow-x-hidden overflow-y-auto', SCROLLBAR_HIDDEN)"
    >
      <q-card-section class="flex flex-center q-mb-none absolute w-full z-10">
        <q-space />
        <q-btn class="text-lg text-white" icon="close" flat round dense v-close-popup />
      </q-card-section>
      ...
      <q-card-section class="q-py-none warning-content-wrapper">
          ...
          <q-list class="agree-list-wrapper">
            <q-item v-for="(item, index) in cmsPopupAgreeList" :key="index" tag="label" v-ripple>
              <q-item-section side top>
                <div class="w-1.5 h-1.5 rounded-full bg-[#F00] mr-2 mt-1"></div>
              </q-item-section>
        ...
        <p class="text-justify text-sm mt-[1.5rem] text-white">
          <template v-if="isNS4M">
            Grup Telegram & Facebook resmi NUS45 menyiapkannotifikasi pembagian Angpao 🧧 serta Bonus Harian.Hanya
            member grup yang memperoleh informasi tersebut.Link grup dapat ditemukan di bagian bawah situs.
          </template>
          <template v-else>
            {{ $t("common.tip.government_own_if_ineligible") }}
          </template>
        </p>
