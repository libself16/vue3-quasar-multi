<template>
  <HeaderTitleBack v-if="$q.platform.is.mobile" titleI18n="member.profile.myDetails" variant="blueOrange">
  ...
  <q-form v-else class="profile-container pc" @submit="setUserInfo2">
    ...
    <div class="profile-body">
      ...
      <template v-for="item in memberColumnList" :key="item.column_name">
        ...
        <div v-else-if="item.type === INPUT_TYPE.Enums.DATE" class="input-container">
          ...
          >
            <template v-slot:before> <q-icon :name="memberColumnIconName[item.column_name]" /> </template>
            <q-menu>
              <q-date v-model="userInfoForm2[item.column_name]" mask="YYYY-MM-DD" minimal class="calendar-content" />
            </q-menu>
          </q-input>
        </div>
      </template>
                ...
                <q-popup-proxy cover transition-show="scale" transition-hide="scale">
                  <q-date v-model="userInfoForm2.self_exclusion_at" :options="dateOptions">
                    <div class="row items-center justify-end">
                      <q-btn v-close-popup :label="$t('common.btn.confirm')" class="calendar-content" flat />
                    </div>
                  </q-date>
                </q-popup-proxy>
              </q-icon>
      ...
      <div class="btn-submit-container">
        <q-btn unelevated class="calendar-content" type="submit">{{ $t("common.btn.submit") }}</q-btn>
      </div>
    </div>
  </q-form>
</template>
...
.profile-container {
  ...
  :deep(.q-field__control):before {
    border-bottom-color: $functional-line !important;
  }
