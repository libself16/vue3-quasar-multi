      ...
      <div v-else class="forget-password-form form-sms">
        <q-form @submit.prevent="forgetPasswordSms">
          <div class="form-content">
            ...
            <div class="action-btns row justify-center">
              <q-btn class="submit-btn" :loading="isLoading"> {{ $t("common.btn.submit") }}</q-btn>
            </div>
          </div>
        </q-form>
      </div>
...
import BannerMember from "app/template/set_r024/components/BannerMember.vue"
const $q = useQuasar()
const Rules = useRule()
const { t } = useI18n()
...
.field-input
  :deep(.q-field__control)
    ...
    background: $functional-input !important
