<template>
  <q-dialog v-model="dialog" persistent>
    <q-card class="age-warning-wrapper overflow-x-hidden overflow-y-auto" :class="cx(
      'age-warning-wrapper',
      ...
      <q-card-section class="flex flex-center q-mb-none absolute w-full z-10">
        <q-space />
        <q-btn class="text-lg close-btn" icon="close" flat round dense v-close-popup />
      </q-card-section>
          ...
          <q-list class="agree-list-wrapper">
            <q-item v-for="(item, index) in cmsPopupAgreeList" :key="index" tag="label" v-ripple>
              <q-item-section side top>
                <div class="w-1.5 h-1.5 rounded-full mr-2 mt-1 bg-[#e38901]"></div>
              </q-item-section>
...
:deep(.q-separator)
  background: $functional-line !important
...
.age-warning-wrapper
  ...
  .warning-content-wrapper
    ...
    :deep(.q-card)
      background: $secondary-card-secondary !important
      color: $neutral-01 !important
      border: 1px solid $functional-line !important
    ...
    .age-verification-title
      @apply text-justify text-sm mt-[1.5rem]
      color: $neutral-01

    .check-all-verification-wrapper
      @apply w-full flex items-center font-medium text-[0.875rem]
      @apply flex-wrap justify-start mt-[0.9375rem]
      color: $neutral-01
      :deep(.q-item)
        width: 100%
        padding: 0.5rem 0
