<template>
  <div class="wallet-dropdown-select">
    <q-select
      ...
      ref="selectRef"
      :menu-offset="[120, 10]"
      :popup-content-style="{ padding: '20px', backgroundColor: '#04275f' }"
      hide-dropdown-icon
      :readonly="!isCash"
    ...
    >
      ...
      <template v-slot:option="{ itemProps, opt, selected, toggleOption }">
        <q-item v-bind="itemProps" class="wallet-list wallet-bg-image">
          <q-item-section side>
            <q-checkbox :model-value="selected" class="pup_check" @update:model-value="toggleOption(opt)" />
          </q-item-section>
...
.wallet-container {
  ...
  :deep(.q-field__inner) {
    ...
    background: rgba($functional-input, 0.7);
