<template>
  <div :class="`${$q.platform.is.mobile ? 'h5' : 'pc'}`" class="phone-forgot-password-wrapper">
    <!-- sms -->
    <q-form v-if="forgetMethod === FORGOT_PASS_METHOD.Enums.Sms" @submit="showVerifyModal([forgetPasswordSms])">
      ...
      <div class="btn-password-container mt-5">
        <q-btn
          ...
          :disable="isLoading"
        >
          <span class="verify-text">{{ $t("common.btn.emailVerification") }}</span>
        </q-btn>
      </div>
    </q-form>
    <!-- account -->
    <q-form v-else @submit="showVerifyModal([forgetPassword])" autocomplete="new-password">
      <div class="form-container">
        <q-input
      ...
      <div class="btn-password-container mt-5">
        <q-btn
          ...
          :disable="isLoading"
        >
          <span class="verify-text">{{ $t("common.btn.smsVerification") }}</span>
        </q-btn>
      </div>
    </q-form>
  </div>
