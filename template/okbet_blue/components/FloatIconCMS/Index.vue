<template>
  <div v-if="floatingIconList?.length > 0" class="float-wrapper w-fit">
    ...
    <div @click="toggleShow" class="icon-container">
      <q-img :src="floatIconPath" :class="`float-cms-icon ${floatIconSizeStyle}`" alt="float-cms-icon-open" />

      <div v-if="isShow">
        <q-img
          :src="floatCmsCloseIconBlue02"
          :class="`
              float-cms-icon
              ${floatIconSizeStyle}
...
import { useEntranceHandler } from "app/template/okbet_blue/composables/useCms"

const { floatingIconList, handleCmsList, shouldDisplayDevice, floatIcon, isLoading } = useCms()
const { nowLang } = useLanguage()
const { handleEntranceClick } = useEntranceHandler()
const { floatCmsOpenIconBlue02, floatCmsCloseIconBlue02 } = useSiteImg()
...
const floatIconPath = computed(() => {
  ...
  return matched && matched.storage_key ? matched.storage_key : floatCmsOpenIconBlue02
})
