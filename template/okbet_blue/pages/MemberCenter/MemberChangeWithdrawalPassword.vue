<template>
  <HeaderTitleBack v-if="$q.platform.is.mobile" titleI18n="member.changePassword.setWithdrawalPassword" variant="blue">
    <template #left>
      <q-btn icon="arrow_back_ios" class="btn-back hide-hover" size="md" flat round dense @click="handleBackBtn" />
    </template>
    <q-form @submit="handleSubmit" class="password-container h5" autocomplete="new-password">
      ...
      <div class="submit-container">
        <BasePrimaryBtn unelevated type="submit" class="btn-submit">{{ $t("common.btn.confirm") }}</BasePrimaryBtn>
      </div>
    </q-form>
  </HeaderTitleBack>
    ...
    <q-card class="password-container pc">
      <q-form @submit="handleSubmit" @reset="resetWithdrawalPasswordForm">
        ...
        <q-card-section class="password-footer">
          <BasePrimaryBtn type="submit">{{ $t("common.btn.submit") }}</BasePrimaryBtn>

          <BasePrimaryOutlineBtn type="reset">{{ $t("common.btn.reset") }}</BasePrimaryOutlineBtn>
        </q-card-section>
      </q-form>
    </q-card>
  </q-dialog>
...
.password-container {
  &.pc {
    ...
    .password-header {
      ...
      .title {
        @apply flex text-white text-[1.75rem] font-semibold;
        line-height: 1.5;
