          ...
          <q-expansion-item class="expansion-wrapper">
            ...
            <q-card>
              <q-card-section class="revenue-wrapper">
                <template v-for="(revenueValue, currencyCode) in item.revenues" :key="currencyCode">
                  <div v-if="getCurrencyCode(Number(currencyCode))" class="revenue-item">
...
import Pagination from "app/template/set_r023/components/Pagination/Index.vue"
...
const getCurrencyCode = (currencyId: number) => {
  return currencyIdMap.value?.[currencyId]?.code ?? null
}
...
@import "app/template/set_r023/assets/css/referral.scss";
</style>
