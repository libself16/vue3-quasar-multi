<template>
  <div class="web-contact-content">
    <ul v-if="contactUsList && contactUsList?.length > 0" class="row text-center contact-us-header_inner">
      <li
        v-for="(cmsItem, cmsIndex) in contactUsList"
        :key="cmsIndex"
...
import { useContactUsListCms } from "src/common/apiHooks/cms/useContactUsListCms"

// const { cmsListState, handleCmsList } = useCms()
const { nowLang } = useLanguage()
const { handleEntranceClick } = useEntranceHandler()
const { contactUsList } = useContactUsListCms()
...
.web-contact-content
  ...
  .contact-us-header_inner
    ...
    .contact_dotted
      ...
      border-right: 1px dotted $border-dark-color
      width: 33%
      font-size: 18px
