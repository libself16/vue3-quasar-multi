<template>
  <div v-if="floatingIconList && floatingIconList?.length > 0" class="float-wrapper w-fit">
    <!-- 切換收合 icon -->
    <div @click="toggleShow" class="icon-container relative">
      <q-img
...
import { useFloatingIconListCms } from "src/common/apiHooks/cms/useFloatingIconListCms"

const { floatIcon, isLoading } = useCms()
// const { floatingIconList, handleCmsList, shouldDisplayDevice } = useCms()
const { nowLang } = useLanguage()
const { handleEntranceClick } = useEntranceHandler()
const { floatCmsOpenIconGray, floatCmsCloseIconGray } = useSiteImg()
const { floatingIconList, shouldDisplayDevice } = useFloatingIconListCms()
...
const floatIconPath = computed(() => {
  ...
  return matched && matched.storage_key ? matched.storage_key : floatCmsOpenIconGray
})
