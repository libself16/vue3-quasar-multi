<template>
  <HeaderTitleBack v-if="$q.platform.is.mobile" titleI18n="member.withdrawal.title" variant="green-member">
    <q-form class="withdraw-layout h5" :class="{ '!pb-[24rem]': isTelegramMiniApp }" @submit="handleWithdralSubmit">
      ...
      <div
        v-if="withdrawState.usingFundType !== FUND_METHOD_TYPE.BackendKeysEnums.ExternalChannelTransfer"
      ...
      <div
        class="withdraw-container"
        v-if="
          withdrawState.uploadConfig.uploadSwitch &&
...
const $q = useQuasar()
const { userInfo2, userWalletMap, getUserInfo2, activeWalletCurrencyCode } = useUserInfo()
const { t } = useI18n()
const {
  gatewayFilterList,
  bankList,
...
const handleWithdralSubmit = async () => {
  ...
  const res = await handlerWithdrawal()
