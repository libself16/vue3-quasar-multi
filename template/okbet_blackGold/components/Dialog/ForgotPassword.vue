    ...
    <q-card :class="`${$q.platform.is.mobile ? 'h5' : 'pc'}`" class="forgot-card">
      ...
      <q-card-section class="card-right">
        <div v-if="$q.platform.is.mobile" class="close-btn-container">
          <q-btn icon="arrow_back_ios" class="hide-hover text-gold" size="md" flat round dense @click="handleBackBtn" />
          <!-- <h2>{{ $t("member.forgotPassword.forgotPassword") }}</h2> -->
          <div class="w-8 h-8 invisible"></div>
        </div>
        <div v-else class="close-btn-container">
          <q-btn icon="close" class="hide-hover text-gold" flat round dense v-close-popup />
        </div>
          ...
          <template v-if="isStep1">
            <div class="password-btn-container">
              <q-btn class="btn-password hide-hover" @click="isStep1 = false">
                <img :src="svgIcon('retrieve-password-black')" alt="" />
                <span>
                  {{
                    isPhoneRegisterMode
              ...
              <q-btn flat class="btn-login hide-hover text-gold" @click="goLogin">
      ...
      .card-right-form {
        ...
        .btn-password {
          ...
          :deep(.q-btn__content) {
            ...
            span {
              @apply text-left break-words whitespace-nowrap text-base;
  ...
  &.h5 {
    ...
    .card-right {
      ...
      .password-btn-container {
        @apply mx-auto p-4 mt-[7.5rem] w-[45rem] rounded-lg flex flex-col justify-start items-center;
        box-shadow: rgba(0, 24, 30, 0.06) 0px 6rem 10rem;
        border: 1px solid $gray-border-color;
        .btn-password {
          @apply text-sm capitalize w-full;
          height: 3.0625rem;
          color: $primary-text-black;
