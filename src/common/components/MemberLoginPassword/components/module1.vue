<template>
  <div
    id="member-login-password-content"
    class="w-full h-full rounded-xl p-5 phone:p-0 flex flex-col gap-[.625rem] bg-[var(--bg-11)] phone:bg-transparent"
  >
    <!-- 標題 -->
    <div id="member-login-password-header" class="flex items-center justify-between gap-[.625rem]">
      <div
        id="member-login-password-header-title"
    ...
    <q-form
      ref="formRef"
      id="member-login-password-main"
      class="w-[35rem] phone:w-full flex-1 flex flex-col justify-between gap-5 mx-auto"
      @submit="handleSubmit"
    >
      <div id="member-login-password-body">
        <div v-if="!accountInfo.empty_password" id="member-login-password-form-content1">
          <div class="member-login-password-form-item">
            <div class="member-login-password-form-title">
              {{ $t("member.forgotPassword.currentPassword") }}
            </div>
            <q-input
              v-model="passwordForm.old_password"
              class="member-login-password-form-input"
        ...
        <div
          id="member-login-password-form-content2"
          class="mt-[.625rem] grid grid-cols-2 gap-[.625rem] phone:grid-cols-1"
          :class="{ '!grid-cols-1': accountInfo.empty_password }"
        >
          <div class="member-login-password-form-item">
            <div class="member-login-password-form-title">
      ...
      <div id="member-login-password-footer" class="flex items-center justify-center">
        <button
          id="member-login-password-submit-btn"
          :class="`
...
const { accountInfo, passwordForm, setUserPassword } = useUserInfo()
const Rules = useRule()
...
.member-login-password-form-item {
